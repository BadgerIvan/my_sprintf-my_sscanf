...
typedef void (*process_func_t)(char *, va_list, fmt_t *, my_size_t *);
...
static char *parse_flags(const char *str, flags_t *flags) {
...
static char *parse_width(const char *str, va_list args, my_size_t *result) {
  char *buffer = (char *)str;
  if (isdigit(*buffer))
    buffer = parse_my_size_t_from_str(buffer, result);
  else if (*buffer == '*') {
    *result = (my_size_t)va_arg(args, my_size_t);
    buffer++;
  }
  return buffer;
}

static char *parse_precision(const char *str, va_list args, my_size_t *result,
                             int *result_set) {
...
static char *parse_length(const char *str, char *result) {
  char *buffer = (char *)str;
  if (*str == 'h' || *str == 'l' || *str == 'L') *result = *buffer++;
  return buffer;
}

static char *parse_type(const char *str, char *result) {
  char *buffer = (char *)str;
  *result = *buffer;
  return ++buffer;
}

static char *parse_all_in_flag(const char *str, va_list args, fmt_t *fmt) {
  char *buffer = (char *)str;
...
static void format_char_with_width_and_left_alignment(char *buffer, char ch,
                                                      my_size_t width,
                                                      my_size_t *len) {
  buffer[(*len)++] = ch;
  my_memset(buffer + *len, ' ', width - 1);
  *len += width - 1;
}

static void format_char_with_width(char *buffer, char ch, my_size_t width,
                                   my_size_t *len) {
  my_memset(buffer + *len, ' ', width - 1);
  *len += width - 1;
  buffer[(*len)++] = ch;
...
static void format_char(char *buffer, char ch, my_size_t *len) {
  buffer[(*len)++] = ch;
}

static void process_char(char *buffer, va_list args, fmt_t *fmt,
                         my_size_t *len) {
...
static my_size_t calculate_string_len(const char *str, fmt_t *fmt) {
  my_size_t result = my_strlen(str);
  if (fmt->precision_set)
    result = result > fmt->precision ? fmt->precision : result;
  return result;
}

static my_size_t calculate_padding(fmt_t *fmt, my_size_t string_len) {
  my_size_t padding = 0;
...
static my_size_t format_string_with_left_alignment(char *buffer,
  ...
  offset += string_len;
  if (padding) {
    my_memset(buffer + string_len, ' ', padding);
    offset += padding;
  }
  return offset;
}
...
static my_size_t format_string(char *buffer, const char *string,
  ...
  my_memcpy(buffer + padding, string, string_len);
  offset += string_len;
  return offset;
}
...
static void process_string(char *buffer, va_list args,
  ...
  my_size_t padding = calculate_padding(fmt, string_len);
...
static void handle_zero_precision_case(char *num_string, my_size_t num,
                                       fmt_t *fmt) {
  if (fmt->precision_set && fmt->precision == 0 && num == 0)
    num_string[0] = '\0';
}

static my_size_t get_num_zeros(my_size_t len_buffer, fmt_t *fmt) {
  my_size_t num_zeros = 0;
  if (fmt->precision_set && fmt->precision > len_buffer)
    num_zeros = fmt->precision - len_buffer;
  return num_zeros;
}

static my_size_t get_padding(my_size_t total_len, fmt_t *fmt) {
...
static my_size_t format_integer_with_left_alignment(
    char *buffer, const char *num_string, char sign, my_size_t len_buffer,
    my_size_t num_zeros, my_size_t padding) {
  my_size_t offset = 0;
  if (sign) buffer[offset++] = sign;
  if (num_zeros) {
    my_memset(buffer + offset, '0', num_zeros);
    offset += num_zeros;
  }
  my_memcpy(buffer + offset, num_string, len_buffer);
  offset += len_buffer;
  if (padding) {
    my_memset(buffer + offset, ' ', padding);
    offset += padding;
  }
  return offset;
}

static my_size_t format_integer_without_left_alignment(
    char *buffer, const char *num_string, char sign, my_size_t len_buffer,
    my_size_t num_zeros, my_size_t padding, char fill) {
  my_size_t offset = 0;
  if (fill == ' ') {
    my_memset(buffer + offset, fill, padding);
    offset += padding;
  }
  if (sign) buffer[offset++] = sign;
  if (fill == '0') {
...
static void format_integer(char *buffer, char *num_string, my_size_t num,
                           int is_negative, fmt_t *fmt, my_size_t *len) {
  char sign = get_sign(is_negative, fmt);
  handle_zero_precision_case(num_string, num, fmt);
  my_size_t len_buffer = my_strlen(num_string);
  my_size_t num_zeros = get_num_zeros(len_buffer, fmt);

  my_size_t total_len = len_buffer + num_zeros;
  if (sign) total_len++;

  my_size_t padding = get_padding(total_len, fmt);
...
static void process_int(char *buffer, va_list args, fmt_t *fmt,
                        my_size_t *len) {
  long int num = 0;
  if (fmt->length == 'h')
  ...
  my_size_t_to_string(num_string, num);
...
static void process_unsigned_integer(char *buffer, int base, int upper,
  ...
  my_size_t_to_string_with_base(num_string_with_prefix, num, base);
  if (upper) to_upper_string(num_string);
  format_integer(buffer, num_string, num, 0, fmt, len);
}

static void process_unsigned_int(char *buffer, va_list args, fmt_t *fmt,
                                 my_size_t *len) {
  process_unsigned_integer(buffer, 10, NOT_UPPER, args, fmt, len);
}
static void process_hex_int(char *buffer, va_list args, fmt_t *fmt,
                            my_size_t *len) {
  process_unsigned_integer(buffer, 16, NOT_UPPER, args, fmt, len);
}

static void process_hex_int_with_upper(char *buffer, va_list args, fmt_t *fmt,
                                       my_size_t *len) {
  process_unsigned_integer(buffer, 16, UPPER, args, fmt, len);
}

static void process_octal_int(char *buffer, va_list args, fmt_t *fmt,
                              my_size_t *len) {
...
static void process_pointer(char *buffer, va_list args, fmt_t *fmt,
                            my_size_t *len) {
...
static void process_procent(char *buffer, __attribute__((unused)) va_list args,
                            __attribute__((unused)) fmt_t *fmt,
                            my_size_t *len) {
  buffer[(*len)++] = '%';
}

static void process_number(__attribute__((unused)) char *buffer, va_list args,
                           fmt_t *fmt, my_size_t *len) {
  long int *num = 0;
  if (fmt->length == 'l')
...
static void resolve_precision_flag_for_float(fmt_t *fmt) {
  if (fmt->precision_set) return;
  fmt->precision = 6;
  fmt->precision_set = 1;
}

static void resolve_flags_for_float(fmt_t *fmt) {
  fmt->precision_set = 0;
  fmt->precision = 0;
}

static void float_to_string_with_mode(char *num_string, long double num,
                                      my_size_t precision, int mode,
                                      fmt_t *fmt) {
  if (mode == FLT_FULL)
...
static void format_float(char *buffer, va_list args, fmt_t *fmt, my_size_t *len,
  ...
  resolve_precision_flag_for_float(fmt);
  char num_string[1000];
  float_to_string_with_mode(num_string, num, fmt->precision, mode, fmt);
  if (upper) to_upper_string(num_string);
  resolve_flags_for_float(fmt);
  format_integer(buffer, num_string, 1, is_neg, fmt, len);
}

static void process_float(char *buffer, va_list args, fmt_t *fmt,
                          my_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_FULL);
}

static void process_float_with_upper(char *buffer, va_list args, fmt_t *fmt,
                                     my_size_t *len) {
  format_float(buffer, args, fmt, len, UPPER, FLT_FULL);
}

static void process_float_scien(char *buffer, va_list args, fmt_t *fmt,
                                my_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_SCIEN);
}

static void process_float_scien_with_upper(char *buffer, va_list args,
                                           fmt_t *fmt, my_size_t *len) {
  format_float(buffer, args, fmt, len, UPPER, FLT_SCIEN);
}

static void process_float_auto(char *buffer, va_list args, fmt_t *fmt,
                               my_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_AUTO);
}

static void process_float_auto_with_upper(char *buffer, va_list args,
                                          fmt_t *fmt, my_size_t *len) {
  format_float(buffer, args, fmt, len, UPPER, FLT_AUTO);
}

static void register_all_funcs(process_func_t *array_of_funcs) {
  ...
  register_process_func(array_of_funcs, 'p', process_pointer);
  register_process_func(array_of_funcs, '%', process_procent);
  register_process_func(array_of_funcs, 'n', process_number);
...
static void process_type(process_func_t *array_of_funcs, char *buffer,
                         va_list args, fmt_t *fmt, my_size_t *len) {
