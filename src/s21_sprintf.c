...
typedef void (*process_func_t)(char *, va_list, fmt_t *, s21_size_t *);
...
void process_char(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len) {
...
char *get_processed_string(char *str, fmt_t *fmt) {
...
s21_size_t calculate_string_len(const char *str, fmt_t *fmt) {
...
s21_size_t calculate_padding(fmt_t *fmt, s21_size_t string_len) {
...
void process_string(char *buffer, va_list args,
                    __attribute__((unused)) fmt_t *fmt, s21_size_t *len) {
...
char get_sign(int is_negative, fmt_t *fmt) {
  char sign = 0;
  if (is_negative)
    sign = '-';
...
void handle_zero_precision_case(char *num_string, s21_size_t num, fmt_t *fmt) {
  if (fmt->precision_set && fmt->precision == 0 && num == 0)
    num_string[0] = '\0';
}

s21_size_t get_num_zeros(s21_size_t len_buffer, fmt_t *fmt) {
...
s21_size_t get_padding(s21_size_t total_len, fmt_t *fmt) {
  return fmt->width > total_len ? fmt->width - total_len : 0;
}

char get_fill(fmt_t *fmt) {
...
void format_integer(char *buffer, char *num_string, s21_size_t num,
                    int is_negative, fmt_t *fmt, s21_size_t *len) {
...
void process_int(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len) {
  long int num = 0;
  if (fmt->length == 'h')
...
char *make_prefix_for_base(char *buffer, int base, fmt_t *fmt) {
...
void process_unsigned_int(char *buffer, va_list args, fmt_t *fmt,
                          s21_size_t *len) {
  process_unsigned_integer(buffer, 10, NOT_UPPER, args, fmt, len);
}
void process_hex_int(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len) {
  process_unsigned_integer(buffer, 16, NOT_UPPER, args, fmt, len);
}

void process_hex_int_with_upper(char *buffer, va_list args, fmt_t *fmt,
                                s21_size_t *len) {
  process_unsigned_integer(buffer, 16, UPPER, args, fmt, len);
}

void process_octal_int(char *buffer, va_list args, fmt_t *fmt,
                       s21_size_t *len) {
...
void process_pointer(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len) {
...
void process_procent(char *buffer, __attribute__((unused)) va_list args,
                     __attribute__((unused)) fmt_t *fmt, s21_size_t *len) {
  buffer[(*len)++] = '%';
}

void process_number(__attribute__((unused)) char *buffer, va_list args,
                    fmt_t *fmt, s21_size_t *len) {
  long int *num = 0;
  if (fmt->length == 'l')
...
void float_to_string_with_mode(char *num_string, long double num,
                               s21_size_t precision, int mode, fmt_t *fmt) {
  if (mode == FLT_FULL)
...
void format_float(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len,
                  int upper, int mode) {
  long double num = 0;
  if (fmt->length == 'L')
...
void process_float(char *buffer, va_list args, fmt_t *fmt, s21_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_FULL);
}

void process_float_with_upper(char *buffer, va_list args, fmt_t *fmt,
                              s21_size_t *len) {
  format_float(buffer, args, fmt, len, UPPER, FLT_FULL);
}

void process_float_scien(char *buffer, va_list args, fmt_t *fmt,
                         s21_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_SCIEN);
}

void process_float_scien_with_upper(char *buffer, va_list args, fmt_t *fmt,
                                    s21_size_t *len) {
  format_float(buffer, args, fmt, len, UPPER, FLT_SCIEN);
}

void process_float_auto(char *buffer, va_list args, fmt_t *fmt,
                        s21_size_t *len) {
  format_float(buffer, args, fmt, len, NOT_UPPER, FLT_AUTO);
}

void process_float_auto_with_upper(char *buffer, va_list args, fmt_t *fmt,
                                   s21_size_t *len) {
