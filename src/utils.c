...
void my_size_t_to_string(char *str, my_size_t num) {
  my_size_t_to_string_with_base(str, num, 10);
}
...
static long double get_rounder(my_size_t precision) {
  long double rounder = 0.5L;
  for (my_size_t i = 0; i < precision; i++) {
    rounder /= 10.0L;
  }
  return rounder;
}
...
static int make_buffer_if_nan_or_inf(char *str, long double num) {
  int flag = 0;
  if (isnan(num)) {
    my_strncpy(str, is_negative(num) ? "-nan" : "nan", 5);
    flag = 1;
  } else if (isinf(num) || num == INFINITY || num == -INFINITY) {
    my_strncpy(str, is_negative(num) ? "-inf" : "inf", 5);
    flag = 1;
  }
  return flag;
}

void long_double_to_string(char *str, long double num, my_size_t precision) {
...
static my_size_t get_precision(my_size_t precision) {
  if (precision == 0) precision = 1;
  return precision;
}

static int is_scientific(int exponent, my_size_t precision) {
...
static void process_auto_if_scientific(char *buffer, long double num,
  ...
  __long_double_to_scientific(buffer, num, precision - 1);
  char *e_pos = my_strchr(buffer, 'e');
  char *dot_pos = my_strchr(buffer, '.');
  if (dot_pos < e_pos) {
    char *ptr = e_pos - 1;
...
static void process_auto_if_not_scientific(char *buffer, long double num,
                                           int exponent, my_size_t precision,
                                           int hash_flag) {
  if (hash_flag) {
  ...
  __long_double_to_string(
      buffer, num, (exponent < 0) ? precision - 1 : precision - 1 - exponent);
  char *dot_pos = my_strchr(buffer, '.');
  char *ptr = buffer + my_strlen(buffer) - 1;
